...
abstract class RepeaterFieldAction
{
    ...
    protected function isVisible(CustomForm $record,array $typeClosers, array $state, array $arguments): bool {
        foreach ($typeClosers as $closer) if($closer($record, $state, $arguments)) return true;
        return false;
    }


    public static function getDefaultTypeClosure(CustomFieldType $type): Closure {
        return function (CustomForm $form, $state,$arguments) use ($type):bool {
