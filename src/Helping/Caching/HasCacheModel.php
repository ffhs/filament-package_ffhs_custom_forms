...
trait HasCacheModel
{
    ...
    {
        if (!property_exists(static::class, 'cacheWith')) {
            return [];
        }
    ...
    public function setCacheValue(string $key, mixed $value): void
    {
        ...
        if ($value instanceof RelationCachedInformations) {
            $value = $value->getModels();
        } //ToDo decide
    ...
    public function isCaching(): bool
    {
        ...
        if ($this->useCache instanceof Closure) {
            return ($this->useCache)($this);
        }
    ...
    {
        if (!property_exists(static::class, 'defaultCaching')) {
            return true;
        }
...
//    }
    ...
    public function getRelationCached($name): mixed
    {
        ...
        if (method_exists($this, $cacheMethodeName)) {
            $result = $this->$cacheMethodeName();
        } else {
            $relation = $this->$name();
            //Belongs to
            if ($relation instanceof BelongsTo) {
                $result = $this->getCachedBelongsTo($relation);
            } else {
                if ($relation instanceof HasOne) {
                    $result = $this->getCachedHasOne($relation);
                } //Many RelationShips
                else {
                    $result = $this->getOtherCachedRelation($name);
                }
            }
        }

        if ($result instanceof RelationCachedInformations) {
...
//    }
    ...
    protected function getOtherCachedRelation(string $name): mixed
    {
        ...
        return Cache::remember($cacheKey, static::getCacheDuration(), function () use ($name) {
            ...
            if (is_null($result) || !($related instanceof CachedModel)) {
                return $result;
            }
